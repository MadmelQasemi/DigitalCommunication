...
if sound_card
    ...
    % cutting the relevant part of the message 
    % cheating by using the length of sTX
    
    extractedMsg = cutOffMsg(sRX,sTX); 
else
    extractedMsg = sTX; 
end
...
[synchedReal, synchedImaginary, clockImaginary ] = synchronization(yReal, yImaginary, fsa, alpha, k, barkerCode);
...
figures(symbols, signalReal, signalImaginary, yReal, yImaginary, Tsa, Tsym, Nsym, Nsam, alphas); 
